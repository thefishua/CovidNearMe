...
RESOURCE_CODE = "21304414-1ff1-4243-a5d2-f52778048b29"
...
def casesActivePerLGA_SQL(lga_code: int) -> int:
    today = date.today()
    past = today - timedelta(days=COVID_INFECTION_PERIOD)
    location = BASE + RESOURCE_SQL
    s_context = ssl.SSLContext()
    cases = 0
    query = f"SELECT * from \"{RESOURCE_CODE}\" WHERE lga_code19 = \"jones\" "
    query += "AND notification_date BETWEEN \"{past}\" AND \"{today}\""
    # print(yelp)
